...
def cla():
    ...
    [data-testid="stSidebar"] {
        background-color: #0A3D62;   /* màu nền sidebar */
        color: #CC9933;              /* màu chữ trong sidebar */
    }
    [data-testid="stSidebar"] * {
        color: #CC9933;              /* toàn bộ text trong sidebar */
    }
    </style>
    """
...
def pre():
    ...
    with st.sidebar.expander("≡ Menu", expanded=True):
        ...
        if st.button("Tỷ giá USD/VND", key="btn_tygia"):
            st.session_state.selected_function = "Ty_gia"
        if st.button("Phân tích tương quan", key="btn_tuongquan"):
    ...
    if st.session_state.selected_function == "Gia_vang":
        ...
        st.pyplot(phan_tich_tuong_quan())